	...
	if (Core_Array::getGet('filter') || Core_Array::getGet('sorting'))
	{
		...
		if ($price_from || $price_to || $sorting == 1 || $sorting == 2)
		{
			...
			foreach ($aShop_Currencies as $oShop_Currency)
			{
				...
				$query_currency_switch = "IF (`shop_items`.`shop_currency_id` = '{$oShop_Currency->id}', IF (COUNT(`shop_discounts`.`id`), (`price` - `price` * SUM(IF(`shop_discounts`.`type` = 0, `shop_discounts`.`value`, 0)) / 100) * {$currency_coefficient} - SUM(IF(`shop_discounts`.`type`, `shop_discounts`.`value`, 0)), `shop_items`.`price` * {$currency_coefficient}), {$query_currency_switch})";
			}
